'''
Code adopted from https://github.com/FlyingGiraffe/vnn/blob/master/models/vn_layers.py
'''
import os
import sys
import copy
...
class LocalFeatureEncoder(MessagePassing):
    def __init__(self, **kwargs):
        kwargs.setdefault('aggr', 'mean')
        super().__init__(**kwargs)

    def forward(self, pos: Union[Tensor, PairTensor], edge_index: Adj) -> Tensor:
        """"""
        ...
        out = self.propagate(edge_index, pos=pos, size=None)

        return out

    def message(self, pos_i: Tensor, pos_j: Tensor) -> Tensor:
        # j -> i
        pos_j = pos_j.unsqueeze(1)
        pos_i = pos_i.unsqueeze(1)
        rel_pos = pos_j - pos_i
        cross = torch.cross(pos_j, pos_i, dim=2)
        msg = torch.cat([rel_pos, cross, pos_i], dim=1).view(pos_j.size(0), -1)
        return msg
